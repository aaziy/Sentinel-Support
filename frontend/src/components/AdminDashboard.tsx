...
/* ── Mock sparkline data ─────────────────────────────────── */
function useMockSparkline(seed: number, points = 20) {
  return useMemo(() => {
    const data: { v: number; label: string }[] = [];
    let v = seed;
    for (let i = 0; i < points; i++) {
      v = Math.max(0, v + Math.floor(Math.random() * 7 - 3));
      data.push({ v, label: `${points - i}m ago` });
    }
    return data;
  }, [seed, points]);
}
...
function LeaderboardRow({ rank, name, resolved, avgTime }: { rank: number; name: string; resolved: number; avgTime: string }) {
  return (
    <div className="flex items-center gap-2 py-1.5">
      <span className={clsx(
        "w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0",
        rank === 1 ? "bg-amber-500/10 text-amber-400" : "bg-zinc-800 text-zinc-500"
      )}>
        {rank}
      </span>
      <div className="flex-1 min-w-0">
      ...
      <span className="text-[10px] font-mono text-emerald-400 tabular-nums">{resolved}</span>
      <span className="text-[9px] font-mono text-zinc-600">{avgTime}</span>
    </div>
  );
}
...
export default function AdminDashboard() {
  ...
  const sparkTotal    = useMockSparkline(all.length + 3);
  const sparkPending  = useMockSparkline(pending.length + 2);
  const sparkResolved = useMockSparkline(resolved.length + 4);
  const sparkRate     = useMockSparkline(all.length ? Math.round((pending.length / all.length) * 100) : 5);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-2 pt-2">
          ...
          <div className="card-surface-dense p-3">
            ...
            <div className="space-y-0 divide-y divide-white/[0.03]">
              <ActivityItem action="AI resolved" detail="password reset query" time="2m ago" icon={<CheckCircle2 className="w-3 h-3 text-emerald-400" />} />
              <ActivityItem action="Escalated" detail="billing dispute to human" time="5m ago" icon={<TrendingUp className="w-3 h-3 text-amber-400" />} />
              <ActivityItem action="New ticket" detail="from user #4821" time="8m ago" icon={<Inbox className="w-3 h-3 text-accent-400" />} />
              <ActivityItem action="SLA warning" detail="ticket #8291" time="14m ago" icon={<AlertTriangle className="w-3 h-3 text-rose-400" />} />
              <ActivityItem action="AI resolved" detail="account unlock" time="21m ago" icon={<CheckCircle2 className="w-3 h-3 text-emerald-400" />} />
            </div>
          </div>
          ...
          <div className="card-surface-dense p-3">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-white font-semibold tabular-nums">142</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-zinc-500 font-mono">Avg. Relevance</span>
                <span className="text-[11px] text-emerald-400 font-semibold tabular-nums">76%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-zinc-500 font-mono">Last Indexed</span>
                <span className="text-[11px] text-zinc-400 font-mono">2h ago</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[10px] text-zinc-500 font-mono">Coverage</span>
                <span className="text-[11px] text-amber-400 font-semibold tabular-nums">68%</span>
              </div>
              {/* Mini bar */}
              <div className="w-full bg-zinc-800 rounded-full h-1.5 mt-1">
                <div className="bg-emerald-400/60 h-1.5 rounded-full" style={{ width: "68%" }} />
              </div>
            </div>
          </div>
          ...
          <div className="card-surface-dense p-3">
            ...
            <div className="space-y-0 divide-y divide-white/[0.03]">
              <LeaderboardRow rank={1} name="AI Agent (Sentinel)" resolved={resolved.length || 18} avgTime="1.2s" />
              <LeaderboardRow rank={2} name="Admin (You)" resolved={3} avgTime="4.5m" />
              <LeaderboardRow rank={3} name="Support Lead" resolved={1} avgTime="12m" />
            </div>
            <div className="flex items-center justify-between mt-2 pt-2 border-t border-white/[0.04]">
              <span className="text-[9px] font-mono text-zinc-600">Total handled today</span>
              <span className="text-[10px] font-semibold text-white tabular-nums">{(resolved.length || 18) + 4}</span>
            </div>
          </div>
        </div>
      </div>
